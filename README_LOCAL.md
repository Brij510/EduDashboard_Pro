...
- `folder-structure.json` (source of truth)

Both visitor and developer views use the same file.

## How Content Works
1. App reads `/folder-structure.json` at runtime.
2. During `npm run dev` and `npm run build`, root `folder-structure.json` is auto-copied to `public/folder-structure.json`.
3. So you only need to update root `folder-structure.json`.
4. Admin can edit content from the UI.
5. Use **Download Folder Structure** in the UI to export updated `folder-structure.json`.
6. Replace root `folder-structure.json` with the downloaded file.
7. Commit and push to GitHub.
8. Vercel redeploys and visitors see the updated structure.
...
3. Make folder/video/file changes.
   - For Google Drive files, choose file type: `PDF`, `DOC`, or `ZIP`.
   - If the Drive link is publicly accessible, the app will try to show a preview image on the file card.
...
7. Replace root `folder-structure.json` with the downloaded file.
...
5. Replace local repo root `folder-structure.json` with downloaded file.
