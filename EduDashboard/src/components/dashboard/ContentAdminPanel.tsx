...
export function ContentAdminPanel({
  ...
  const handleSyncFromCloud = async () => {
    if (!gistUrl) {
      toast({ title: "No Gist URL", variant: "destructive" });
      return;
    }

    setIsSyncing(true);
    const key = gistUrl.trim();
  ...
  const handleCopyForGist = async () => {
    const content = JSON.stringify({ contents, categories, videos }, null, 2);
    navigator.clipboard.writeText(content);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
    const key = gistUrl.trim() || getSyncKey()?.trim() || undefined;
    toast({ title: "Copied to Clipboard" });
    const result = await saveZoneData({ contents, categories, videos }, key);
    if (!result.ok) {
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              disabled={isSyncing}
              className="flex flex-col items-center gap-2 p-4 rounded-xl border border-border bg-muted/30 hover:bg-muted disabled:opacity-50"
            >
              <CloudDownload className="w-6 h-6 text-primary" />
              <span className="text-xs font-medium">{isSyncing ? "Syncing..." : "Sync from Gist"}</span>
            </button>
            <button
              onClick={handleCopyForGist}
              className="flex flex-col items-center gap-2 p-4 rounded-xl border border-border bg-muted/30 hover:bg-muted"
            >
              {copied ? <Check className="w-6 h-6 text-green-500" /> : <Copy className="w-6 h-6 text-primary" />}
              <span className="text-xs font-medium">{copied ? "Copied!" : "Copy for Gist"}</span>
            </button>
          </div>

          <div className="p-4 rounded-xl border border-primary/30 bg-primary/5">
          <h4 className="font-semibold text-foreground mb-2 text-xs uppercase tracking-wider">Instructions:</h4>
          <ol className="text-xs text-muted-foreground space-y-2 list-decimal list-inside">
            <li>Copy data with "Copy for Gist"</li>
            <li>Go to <a href="https://gist.github.com" target="_blank" rel="noreferrer" className="text-primary hover:underline">gist.github.com</a></li>
            <li>Paste into a new Gist and name it <code className="bg-muted px-1 rounded">data.json</code></li>
            <li>Click "Create public gist"</li>
            <li>Click the <strong>"Raw"</strong> button on the file</li>
            <li>Copy the URL from your browser address bar</li>
            <li>Save that URL here in the input above</li>
          </ol>
        </div>
        </div>
      )}
