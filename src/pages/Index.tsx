import { useState, useMemo, useEffect, useCallback, useRef } from "react";
import { Sidebar } from "@/components/dashboard/Sidebar";
import { SearchBar } from "@/components/dashboard/SearchBar";
import { Breadcrumb } from "@/components/dashboard/Breadcrumb";
import { VideoGrid } from "@/components/dashboard/VideoGrid";
import { VideoModal } from "@/components/dashboard/VideoModal";
import { StatsBar } from "@/components/dashboard/StatsBar";
import { SettingsPanel } from "@/components/dashboard/SettingsPanel";
import { NotificationPanel } from "@/components/dashboard/NotificationPanel";
import { FolderExplorer } from "@/components/dashboard/FolderExplorer";
import { categories as initialCategories, videos as initialVideos, initialContents } from "@/data/mockData";
import { Video, Category, ContentItem } from "@/types/dashboard";
import { Menu, LogOut, Folder, Video as VideoIcon, ArrowLeft } from "lucide-react";
import { fetchZoneData } from "@/utils/zoneApi";
import { getPreviewImageForFileUrl } from "@/utils/drive";

interface IndexProps {
  isAdmin: boolean;
  onLogout: () => void;
}

interface SearchMatchItem {
  id: string;
  title: string;
  subtitle: string;
  categoryId: string;
}

const Index = ({ isAdmin, onLogout }: IndexProps) => {
  const [categories, setCategories] = useState<Category[]>(initialCategories);
  const [videos, setVideos] = useState<Video[]>(initialVideos);
  const [contents, setContents] = useState<ContentItem[]>(initialContents);
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [currentFolderId, setCurrentFolderId] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedVideo, setSelectedVideo] = useState<Video | null>(null);
  const [selectedContentVideo, setSelectedContentVideo] = useState<ContentItem | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isSearchDropdownOpen, setIsSearchDropdownOpen] = useState(false);
  const searchWrapRef = useRef<HTMLDivElement>(null);
  // Visitors default to legacy (categories), admins default to folders
  const [viewMode, setViewMode] = useState<"folders" | "legacy">(isAdmin ? "folders" : "legacy");

  useEffect(() => {
    let isMounted = true;
    const loadZoneData = async () => {
      const data = await fetchZoneData();
      if (!isMounted) return;

      if (Array.isArray(data.categories)) {
        setCategories(data.categories);
      }
      if (Array.isArray(data.videos)) {
        setVideos(data.videos);
      }
      if (Array.isArray(data.contents)) {
        setContents(data.contents);
      }
    };

    loadZoneData();
    return () => {
      isMounted = false;
    };
  }, []);

  // Filter videos based on category and search (legacy mode)
  const findCategoryById = useCallback((id: string): Category | null => {
    const visit = (nodes: Category[]): Category | null => {
      for (const node of nodes) {
        if (node.id === id) return node;
        if (node.children?.length) {
          const hit = visit(node.children);
          if (hit) return hit;
        }
      }
      return null;
    };
    return visit(categories);
  }, [categories]);

  const getDescendantCategoryIds = useCallback((id: string): string[] => {
    const node = findCategoryById(id);
    if (!node) return [id];

    const ids: string[] = [id];
    const stack = [...(node.children || [])];
    while (stack.length > 0) {
      const current = stack.pop()!;
      ids.push(current.id);
      if (current.children?.length) {
        stack.push(...current.children);
      }
    }
    return ids;
  }, [findCategoryById]);

  const findParentCategory = useCallback((id: string): Category | null => {
    const visit = (nodes: Category[], parent: Category | null): Category | null => {
      for (const node of nodes) {
        if (node.id === id) return parent;
        if (node.children?.length) {
          const hit = visit(node.children, node);
          if (hit) return hit;
        }
      }
      return null;
    };
    return visit(categories, null);
  }, [categories]);

  const getCategoryPathLabel = useCallback((id: string): string => {
    const path: string[] = [];
    let current = findCategoryById(id);
    while (current) {
      path.unshift(current.name);
      current = findParentCategory(current.id);
    }
    return path.join(" / ");
  }, [findCategoryById, findParentCategory]);

  const filteredVideos = useMemo(() => {
    let result = videos;

    // Filter by category
    if (selectedCategory) {
      const allowedIds = new Set(getDescendantCategoryIds(selectedCategory));
      result = result.filter((video) => allowedIds.has(video.categoryId));
    }

    // Filter by search query
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      result = result.filter(
        (video) =>
          video.title.toLowerCase().includes(query) ||
          video.description.toLowerCase().includes(query)
      );
    }

    return result;
  }, [videos, selectedCategory, searchQuery, getDescendantCategoryIds]);

  const searchMatches = useMemo(() => {
    const query = searchQuery.trim().toLowerCase();
    if (!query) return [] as SearchMatchItem[];

    return videos
      .filter(
        (video) =>
          video.title.toLowerCase().includes(query) ||
          video.description.toLowerCase().includes(query)
      )
      .map((video) => ({
        id: video.id,
        title: video.title,
        subtitle: getCategoryPathLabel(video.categoryId),
        categoryId: video.categoryId,
      }));
  }, [videos, searchQuery, getCategoryPathLabel]);

  // Filter contents based on search
  const filteredContents = useMemo(() => {
    if (!searchQuery) return contents;
    const query = searchQuery.toLowerCase();
    return contents.filter(item => item.name.toLowerCase().includes(query));
  }, [contents, searchQuery]);

  const handlePlayVideo = (video: Video) => {
    if (video.resourceType && video.resourceType !== "video") {
      if (video.videoUrl) {
        window.open(video.videoUrl, "_blank", "noopener,noreferrer");
      }
      return;
    }
    setSelectedVideo(video);
    setSelectedContentVideo(null);
    setIsModalOpen(true);
  };

  const handlePlayContentVideo = (item: ContentItem) => {
    setSelectedContentVideo(item);
    setSelectedVideo(null);
    setIsModalOpen(true);
  };

  const handleMarkWatched = (videoId: string) => {
    setVideos((prev) =>
      prev.map((v) => (v.id === videoId ? { ...v, watched: true } : v))
    );
    if (selectedVideo?.id === videoId) {
      setSelectedVideo({ ...selectedVideo, watched: true });
    }
  };

  const getCategoryName = () => {
    if (!selectedCategory) return "All Content";
    const category = findCategoryById(selectedCategory);
    if (category) return category.name;
    return "Content";
  };

  const getVisibleVisitorFolders = (): Category[] => {
    if (!selectedCategory) return categories;
    const selected = findCategoryById(selectedCategory);
    if (!selected) return categories;
    if (selected.children?.length) return selected.children;
    return [];
  };

  const getDirectMaterialsForSelectedFolder = (): Video[] => {
    if (!selectedCategory) {
      return filteredVideos.filter((video) => video.categoryId === "");
    }
    return filteredVideos.filter((video) => video.categoryId === selectedCategory);
  };

  const handleExitFolder = () => {
    if (!selectedCategory) return;
    const parent = findParentCategory(selectedCategory);
    setSelectedCategory(parent ? parent.id : "");
  };

  const handleSelectSearchResult = (item: SearchMatchItem) => {
    setSelectedCategory(item.categoryId);
    setViewMode("legacy");
    setSearchQuery("");
    setIsSearchDropdownOpen(false);
  };

  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (searchWrapRef.current && !searchWrapRef.current.contains(event.target as Node)) {
        setIsSearchDropdownOpen(false);
      }
    };

    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, []);

  // Convert ContentItem to Video for modal display
  const getVideoFromContentItem = (): Video | null => {
    if (!selectedContentVideo || selectedContentVideo.type !== 'video') return null;
    return {
      id: selectedContentVideo.id,
      title: selectedContentVideo.name,
      description: selectedContentVideo.description || '',
      thumbnail: selectedContentVideo.thumbnail || '',
      videoUrl: selectedContentVideo.videoUrl || '',
      duration: selectedContentVideo.duration || '00:00',
      categoryId: '',
      watched: false,
      createdAt: selectedContentVideo.createdAt,
    };
  };

  const handleUpdateContents = (newContents: ContentItem[]) => {
    setContents(newContents);
    
    // Recursive function to build category tree from folders
    const buildCategoryTree = (parentId: string | null): Category[] => {
      const folders = newContents.filter(item => item.type === 'folder' && item.parentId === parentId);
      return folders.map(folder => {
        const children = buildCategoryTree(folder.id);
        return {
          id: folder.id,
          name: folder.name,
          icon: folder.name.toLowerCase().includes('lecture') ? 'Video' : 
                folder.name.toLowerCase().includes('text') ? 'BookOpen' :
                folder.name.toLowerCase().includes('note') ? 'FileText' :
                folder.name.toLowerCase().includes('class') ? 'GraduationCap' : 'Folder',
          children: children.length > 0 ? children : undefined,
          parentId: folder.parentId || undefined
        };
      });
    };

    const newCategories = buildCategoryTree(null);
    setCategories(newCategories);

    // Sync to legacy videos for compatibility
    const allResources = newContents.filter((item) =>
      item.type === "video" ||
      item.type === "pdf" ||
      item.type === "doc" ||
      item.type === "zip"
    );
    const syncedVideos: Video[] = allResources.map((item) => {
      const isFile =
        item.type === "pdf" || item.type === "doc" || item.type === "zip";
      const fileDescription =
        item.type === "pdf"
          ? "PDF document"
          : item.type === "doc"
            ? "Document file"
            : item.type === "zip"
              ? "ZIP archive"
              : "";
      const fileDuration =
        item.type === "pdf"
          ? "PDF"
          : item.type === "doc"
            ? "DOC"
            : item.type === "zip"
              ? "ZIP"
              : "";
      const fileThumbnail =
        item.type === "pdf"
          ? "https://images.unsplash.com/photo-1517842645767-c639042777db?w=800&q=80"
          : item.type === "doc"
            ? "https://images.unsplash.com/photo-1455390582262-044cdead277a?w=800&q=80"
            : item.type === "zip"
              ? "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?w=800&q=80"
              : "";
      const fileUrl = item.pdfUrl || "";
      const fileThumbnailFromUrl = fileUrl
        ? getPreviewImageForFileUrl(fileUrl)
        : null;

      return {
        id: item.id,
        title: item.name,
        description: item.description || fileDescription,
        thumbnail:
          item.thumbnail ||
          (isFile
            ? fileThumbnailFromUrl || fileThumbnail
            : `https://img.youtube.com/vi/${item.videoUrl?.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&]+)/)?.[1] || ''}/maxresdefault.jpg`),
        videoUrl: isFile ? fileUrl : item.videoUrl || "",
        duration: isFile ? fileDuration : item.duration || "00:00",
        categoryId: item.parentId || "",
        watched: false,
        createdAt: item.createdAt,
        resourceType: isFile ? item.type : "video",
      };
    });
    
    setVideos(prev => {
      return syncedVideos.map(nv => {
        const existing = prev.find(pv => pv.id === nv.id);
        return existing ? { ...nv, watched: existing.watched } : nv;
      });
    });
  };

  return (
    <div className="min-h-[100dvh] bg-background text-foreground overflow-x-hidden">
      {/* Sidebar - Desktop */}
      <div className="hidden lg:block">
        <Sidebar
          categories={categories}
          selectedCategory={selectedCategory}
          onCategorySelect={(id) => {
            setSelectedCategory(id);
            setViewMode("legacy");
          }}
        />
      </div>

      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div
          className="lg:hidden fixed inset-0 z-40 bg-background/80 backdrop-blur-sm"
          onClick={() => setIsMobileMenuOpen(false)}
        >
          <div className="w-[82vw] max-w-sm h-full safe-top safe-bottom" onClick={(e) => e.stopPropagation()}>
            <Sidebar
              categories={categories}
              selectedCategory={selectedCategory}
              onCategorySelect={(id, hasChildren) => {
                setSelectedCategory(id);
                setViewMode("legacy");
                if (!hasChildren) {
                  setIsMobileMenuOpen(false);
                }
              }}
            />
          </div>
        </div>
      )}

      {/* Main Content */}
      <main className="lg:ml-64 min-h-[100dvh] safe-bottom">
        {/* Top Bar */}
        <header className="sticky top-0 z-30 glass-panel border-b border-border/30 px-3 sm:px-4 lg:px-8 py-3 sm:py-4 safe-top safe-left safe-right">
          <div className="flex items-center gap-2 sm:gap-4">
            {/* Mobile Menu Button */}
            <button
              className="lg:hidden p-2 hover:bg-muted rounded-lg transition-colors"
              onClick={() => setIsMobileMenuOpen(true)}
            >
              <Menu className="w-5 h-5" />
            </button>

            {/* Search */}
            <div
              ref={searchWrapRef}
              className="flex-1 min-w-0 max-w-2xl relative"
              onFocusCapture={() => setIsSearchDropdownOpen(true)}
              onBlurCapture={() => {
                requestAnimationFrame(() => {
                  if (
                    searchWrapRef.current &&
                    !searchWrapRef.current.contains(document.activeElement)
                  ) {
                    setIsSearchDropdownOpen(false);
                  }
                });
              }}
            >
              <SearchBar
                value={searchQuery}
                onChange={(value) => {
                  setSearchQuery(value);
                  setIsSearchDropdownOpen(Boolean(value.trim()));
                }}
                placeholder="Search content..."
              />
              {searchQuery.trim() && isSearchDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-card border border-border rounded-xl shadow-2xl max-h-72 overflow-y-auto z-50">
                  {searchMatches.length > 0 ? (
                    <div className="p-2 space-y-1">
                      {searchMatches.map((item) => (
                        <button
                          key={item.id}
                          onClick={() => handleSelectSearchResult(item)}
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-muted transition-colors"
                        >
                          <p className="text-sm font-medium text-foreground line-clamp-1">
                            {item.title}
                          </p>
                          <p className="text-xs text-muted-foreground line-clamp-1">
                            {item.subtitle}
                          </p>
                        </button>
                      ))}
                    </div>
                  ) : (
                    <p className="px-3 py-2 text-sm text-muted-foreground">
                      No matching content found.
                    </p>
                  )}
                </div>
              )}
            </div>

            {/* Actions */}
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              {isAdmin && (
                <div className="hidden md:flex items-center gap-2 mr-2 lg:mr-4">
                  <span className="px-2 py-1 text-xs font-medium bg-primary/20 text-primary rounded-full">
                    Admin
                  </span>
                </div>
              )}
              <NotificationPanel
                isAdmin={isAdmin}
                totalVideos={videos.length}
                watchedVideos={videos.filter((video) => video.watched).length}
                hasSearchQuery={Boolean(searchQuery.trim())}
              />
              
              {/* Settings Panel */}
              <SettingsPanel
                isAdmin={isAdmin}
                categories={categories}
                videos={videos}
                contents={contents}
                onUpdateCategories={setCategories}
                onUpdateVideos={setVideos}
                onUpdateContents={handleUpdateContents}
              />
              
              <button
                onClick={onLogout}
                className="p-2 hover:bg-destructive/20 rounded-lg transition-colors text-muted-foreground hover:text-destructive"
                title="Logout"
              >
                <LogOut className="w-5 h-5" />
              </button>
              <div className="hidden sm:flex w-8 h-8 rounded-full bg-gradient-to-br from-primary to-accent items-center justify-center text-xs font-bold text-primary-foreground">
                {isAdmin ? "AD" : "VS"}
              </div>
            </div>
          </div>
        </header>

        {/* Content */}
        <div className="p-3 sm:p-4 lg:p-8 space-y-5 sm:space-y-6">
          {/* View Mode Toggle - Only show to admins */}
          {isAdmin && (
            <div className="flex items-center gap-4 overflow-x-auto pb-1">
              <div className="flex items-center gap-1 p-1 bg-muted rounded-lg shrink-0">
                <button
                  onClick={() => { setViewMode("folders"); setSelectedCategory(null); }}
                  className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                    viewMode === "folders" 
                      ? "bg-background text-foreground shadow-sm" 
                      : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  <Folder className="w-4 h-4" />
                  Folders
                </button>
                <button
                  onClick={() => setViewMode("legacy")}
                  className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                    viewMode === "legacy" 
                      ? "bg-background text-foreground shadow-sm" 
                      : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  <VideoIcon className="w-4 h-4" />
                  Categories
                </button>
              </div>
            </div>
          )}

          {viewMode === "folders" && isAdmin ? (
            <>
              {/* Folder Explorer - Admin Only */}
              <FolderExplorer
                contents={searchQuery ? filteredContents : contents}
                currentFolderId={searchQuery ? null : currentFolderId}
                onNavigate={setCurrentFolderId}
                onPlayVideo={handlePlayContentVideo}
                isAdmin={isAdmin}
                onUpdateContents={handleUpdateContents}
                categories={categories}
                videos={videos}
                onUpdateCategories={setCategories}
                onUpdateVideos={setVideos}
              />
            </>
          ) : (
            <>
              {/* Legacy Category View */}
              <Breadcrumb
                categories={categories}
                selectedCategory={selectedCategory}
                onNavigate={setSelectedCategory}
              />

              <StatsBar videos={filteredVideos} />

              {!isAdmin && (
                <>
                  {(() => {
                    const visibleFolders = getVisibleVisitorFolders();
                    const directMaterials = getDirectMaterialsForSelectedFolder();
                    const hasFolders = visibleFolders.length > 0;
                    const hasDirectMaterials = directMaterials.length > 0;

                    return (
                      <>
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-foreground">
                        {getCategoryName()}
                      </h2>
                      <p className="text-muted-foreground text-sm mt-1">
                        {hasFolders
                          ? `${visibleFolders.length} folder${visibleFolders.length !== 1 ? "s" : ""}${hasDirectMaterials ? `, ${directMaterials.length} material${directMaterials.length !== 1 ? "s" : ""}` : ""}`
                          : `${directMaterials.length} video${directMaterials.length !== 1 ? "s" : ""} available`}
                      </p>
                    </div>
                    <button
                      onClick={handleExitFolder}
                      disabled={!selectedCategory}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium bg-muted/50 border border-border hover:bg-muted transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <ArrowLeft className="w-3.5 h-3.5" />
                      Exit Folder
                    </button>
                  </div>

                  {hasFolders && (
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                      {visibleFolders.map((folder) => (
                        <button
                          key={folder.id}
                          onClick={() => setSelectedCategory(folder.id)}
                          className={`text-left px-3 py-2 rounded-lg border transition-colors ${
                            selectedCategory === folder.id
                              ? "bg-primary text-primary-foreground border-primary"
                              : "bg-muted/40 border-border hover:bg-muted"
                          }`}
                        >
                          <span className="text-sm font-medium truncate block">
                            {folder.name}
                          </span>
                        </button>
                      ))}
                    </div>
                  )}

                  {hasDirectMaterials && (
                    <VideoGrid
                      videos={directMaterials}
                      onPlayVideo={handlePlayVideo}
                    />
                  )}

                  {!hasFolders && !hasDirectMaterials && (
                    <VideoGrid
                      videos={[]}
                      onPlayVideo={handlePlayVideo}
                    />
                  )}
                      </>
                    );
                  })()}
                </>
              )}

              {isAdmin && (
                <>
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-foreground">
                        {getCategoryName()}
                      </h2>
                      <p className="text-muted-foreground text-sm mt-1">
                        {filteredVideos.length} video
                        {filteredVideos.length !== 1 ? "s" : ""} available
                      </p>
                    </div>
                  </div>
                  <VideoGrid videos={filteredVideos} onPlayVideo={handlePlayVideo} />
                </>
              )}
            </>
          )}
        </div>
      </main>

      {/* Video Modal */}
      <VideoModal
        video={selectedVideo || getVideoFromContentItem()}
        isOpen={isModalOpen}
        onClose={() => {
          setIsModalOpen(false);
          setSelectedVideo(null);
          setSelectedContentVideo(null);
        }}
        onMarkWatched={handleMarkWatched}
      />
    </div>
  );
};

export default Index;
