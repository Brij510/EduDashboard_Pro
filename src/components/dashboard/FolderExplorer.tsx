...
export function FolderExplorer({ 
  ...
  const handleItemClick = (item: ContentItem) => {
    ...
    if (item.type === 'folder') {
      ...
      onPlayVideo(item);
    } else if ((item.type === 'pdf' || item.type === 'doc' || item.type === 'zip') && item.pdfUrl) {
  ...
  const getFilePreviewImage = (item: ContentItem) => {
    if (item.type !== "pdf" && item.type !== "doc" && item.type !== "zip") {
      return null;
    }
    return item.pdfUrl ? getPreviewImageForFileUrl(item.pdfUrl) : null;
  };
  ...
  const handleDelete = (id: string) => {
    ...
    collectChildren(id);
    onUpdateContents(contents.filter((item) => !idsToDelete.has(item.id)));
    setContextMenuId(null);
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.contents && Array.isArray(data.contents)) {
          const importPrefix = `imported-${Date.now()}`;
          const idMap = new Map<string, string>();
          data.contents.forEach((item: ContentItem) => {
            idMap.set(item.id, `${importPrefix}-${item.id}`);
          });

          const remappedContents = data.contents.map((item: ContentItem) => ({
            ...item,
            id: idMap.get(item.id) || `${importPrefix}-${item.id}`,
            parentId:
              item.parentId === null
                ? currentFolderId
                : idMap.get(item.parentId) || currentFolderId,
          }));
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-4 xl:gap-6 h-full min-h-0">
          ...
          {isAdmin && (
            <div className="flex items-center gap-2 overflow-x-auto max-w-full pb-1">
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium bg-primary/20 text-primary hover:bg-primary/30 transition-colors shrink-0"
              >
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium bg-green-500/20 text-green-600 hover:bg-green-500/30 transition-colors shrink-0"
              >
      ...
      {/* New Folder Input */}
      {showNewFolderInput && (
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 p-3 bg-muted rounded-lg">
      ...
      {/* Content Grid */}
      <div className="grid grid-cols-1 min-[420px]:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
            ...
            >
              {item.type === 'video' && item.videoUrl ? (
                ...
                <div className="w-full h-full relative">
                  {getFilePreviewImage(item) ? (
                    <img
                      src={getFilePreviewImage(item) || ""}
                      alt={item.name}
                      className="w-full h-full object-cover"
                      onError={(e) => {
                        (e.target as HTMLImageElement).style.display = "none";
            ...
            <div className="p-3">
              ...
              {(item.type === 'pdf' || item.type === 'doc' || item.type === 'zip') && (
                <p className="text-xs text-red-500 text-center mt-1">
                  {item.type.toUpperCase()} - Opens in new tab
                </p>
              )}
            </div>
          </div>
      ...
      {isAdmin && showAdminSidebar && (
        <div className="w-full xl:w-[350px] shrink-0 glass-panel border border-border/30 rounded-xl overflow-hidden hidden xl:block animate-slide-in-right">
          <ContentAdminPanel
            contents={contents}
